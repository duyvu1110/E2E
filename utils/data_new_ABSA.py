...
def words_to_tokens(tokenizer, doc_tokens, max_text_len, model_type): # doc_tokens,表示text按空格划分之后的list
    """
    功能: 将word list 变成 token list, word_list表示
    tok_to_orig_index: token到原来word位置的映射
    all_doc_tokens: tokenizer之后的tokens
    """
    bert_append_list = ['[CLS]', '[SEP]', '[PAD]']
    roberta_append_list = ['<s>', '</s>', '[PAD]']
    if model_type=='bert':
        append_list = bert_append_list
    elif model_type=='roberta':
        append_list = roberta_append_list
    tok_to_orig_index = []
    # all_doc_tokens = ['[unused1]', '[CLS]']
    all_doc_tokens = [append_list[0]]
    ...
    all_doc_tokens.append(append_list[1])
    ...
    for i in range(max_text_len - len(tok_to_orig_index)):
        ...
        all_doc_tokens.append(append_list[2])
    # stop()
...
def process_line(args, text_line, label_line, tokenizer: AutoTokenizer, sample_id):
    ...
    token_offset, tokens = words_to_tokens(tokenizer, text_list, args.max_text_length, args.model_type)
    token_ids = tokenizer.convert_tokens_to_ids(tokens) 
    sample = {'token_ids': token_ids, 'labels': [], 'sample_id': sample_id}

    if have_triples == 0: # 后三都是1
        return sample

    for tri in raw_labels:
        # tri: [sub, obj, aspect, opinion, sentiment]
        # data_path:主要是为了区分中英文数据集，原始数据中，存放格式不一致
        aspect_offset = proc_raw_offset(tri[0], text, args.data_path) # pro_raw_offset: obtain the start offset and end offset for each element
        view_offset = proc_raw_offset(tri[1], text, args.data_path)
        sentiment_label = have_triples * EMO_MAP[int(tri[2])]

        if 'lap' in args.data_path or 'res' in args.data_path:
            # tokens_output.word_to_tokens（k）,其中k表示的是word在text中的位置，位置从0开始计数
            token_offset, tokens = words_to_tokens(tokenizer, text.split(" "), args.max_text_length, args.model_type)
            token_ids = tokenizer.convert_tokens_to_ids(tokens) 
            sample = {'token_ids': token_ids, 'labels': [], 'sample_id': sample_id}
...
def load_data(args, mode: str):
    raw_data = []
    with open(os.path.join(args.data_path, f'{mode}_processed.txt'), 'r') as f:
        for line in f:
