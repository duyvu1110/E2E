...
class Trainer_logits(nn.Module):
    ...
    def train_model(self):
        ...
        for epoch in range(self.args.max_epoch):
            ...
            for batch_id, (input_ids, targets, _, re_logits) in enumerate(tqdm(train_loader, desc=f'training on epoch {epoch}')):
                attention_mask = (input_ids != self.args.tokenizer.pad_token_id).long()
                loss, _ = self.model(input_ids, attention_mask, targets, re_logits) # 将re_logits传入
                # stop()
    ...
    def eval_model(self, eval_loader, process):
        ...
        if process=='dev':
            ...
            if self.args.stage.lower() == "one" or self.args.data_type == 'tuple':
                return metric_absa(prediction, gold)
            elif self.args.stage.lower() == "two" or self.args.data_type == 'quintuple':
