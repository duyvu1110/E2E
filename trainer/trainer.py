...
class Trainer(nn.Module):
    ...
    def train_model(self):
        ...
        if self.args.stage.lower() == "one":
            path = os.path.join(self.args.output_path, 'ckpt-three')
            if not os.path.exists(path):
                os.makedirs(path) 
        elif self.args.stage.lower() == "two":
            path = os.path.join(self.args.output_path, 'ckpt-five')
            if not os.path.exists(path):
                os.makedirs(path) 
        ...
        for epoch in range(self.args.max_epoch):
            ...
            if no_improvement_count >= 10:
                print("No improvement in F1 for 10 consecutive epochs. Early stopping...")
                break               
