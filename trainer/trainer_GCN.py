...
# from trainer.optimizer import AdamW
...
class Trainer_GCN(nn.Module):
        ...
        grouped_params = [
            {
                ...
                'names': [n for n, p in self.model.named_parameters() if 
                            not any(nd in n for nd in no_decay) and component[0] in n],
            ...
            },
            {
                ...
                'names': [p for n, p in self.model.named_parameters() if
                           not any(nd in n for nd in no_decay) and component[1] in n],
                'weight_decay': 0.0,
                'lr': args.decoder_lr
            }
    ...
    def train_model(self):
        ...
        try:
            ...
            os.makedirs(os.path.join(self.args.output_path, 'ckpt-five'))
            print("文件地址已重建")       
        # os.makedirs(os.path.join(self.args.output_path, 'ckpt')) # '--output_path', default='log/'
        ...
        for epoch in range(self.args.max_epoch):
            ...
            # self.model, self.optimizer = amp.initialize(self.model, self.optimizer, opt_level="O1") # add new

            print("=== Epoch %d train ===" % epoch, flush=True) # flush=True 会在print结束之后,立即将内存中的东西显示到屏幕上,清空缓存
            avg_loss = AverageMeter()
            for batch_id, (input_ids, targets, _, dependency_graph) in enumerate(tqdm(train_loader, desc=f'training on epoch {epoch}')):
                attention_mask = (input_ids != self.args.tokenizer.pad_token_id).long()
                loss, _ = self.model(input_ids, attention_mask, dependency_graph, targets)
            ...
            # result = self.eval_model(self.data['test'])
            # test_f1 = result['f1']
            if f1 > best_f1:
        ...
        with open(os.path.join(self.args.output_path,'params.txt'),"a") as f:
    ...
    def eval_model(self, eval_loader, process):
        ...
        with torch.no_grad():
            ...
            for batch_id, (input_ids, target, info, dependency_graph) in enumerate(tqdm(eval_loader, f'evaluation')):
                ...
                gen_triples = self.model.gen_triples(input_ids, attention_mask, info, dependency_graph)
        ...
        # write to file
        with open(os.path.join(self.args.output_path, 'preds.txt'), 'w', encoding='utf-8') as f:
