...
class SeqEncoder(nn.Module):
    def __init__(self, args):
        ...
        self.args = args
        self.bert = AutoModel.from_pretrained('vinai/phobert-base-v2', vocab_size = 64001, ignore_mismatched_sizes=True)
...
class SeqEncoder_last(nn.Module):
    def __init__(self, args):
        super(SeqEncoder, self).__init__()
        self.args = args
        self.bert = AutoModel.from_pretrained(args.bert_directory, vocab_size = 64001, ignore_mismatched_sizes=True)
        self.config = self.bert.config
